...
def conv_op(input_op, name, kh, kw, n_out, dh, dw, p):
    ...
    with tf.name_scope(name) as scope:
        kernal = tf.get_variable(
            scope + 'w', shape=[kh, kw, n_in, n_out],dtype=tf.float32,
...
def run_benchmark():
    with tf.Graph().as_default():
        image_size = 224
        images = tf.Variable(
            tf.random_normal([batch_size, image_size, image_size, 3], dtype=tf.float32, stddev=1e-1)
        )
